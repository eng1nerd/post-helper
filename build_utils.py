...
def oh_never_mind(what):
    state = BuildState(what, PACKAGE_NAME)
    state.oh_never_mind()
...
def has_source_code_tree_changed(task_name, expect_file=None):
    if expect_file:
        ...
        if not os.path.isfile(expect_file):
            # output file gone
            return True
    state = BuildState(task_name, os.path.join(SRC, PACKAGE_NAME))
...
def execute_get_text(command):  # type: (str) ->str
    ...
    """
    try:
        completed = subprocess.run(command, check=True, shell=True, stdout=subprocess.PIPE,)
